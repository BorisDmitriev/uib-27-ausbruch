...
describe('Hello World Box', () => {
    ...
    it("`.helloworld` box is centered horizontally using CSS `margin` property", async () => {
        const textAlign = await page.$eval('.helloworld', el => getComputedStyle(el).textAlign);
        const margin = await page.$eval('.helloworld', el => getComputedStyle(el).margin);
    ...
    it("`.helloworld` box is positioned '7px' from top and '-5px' from left", async () => {
        ...
        const boxLeft = await page.$eval('.helloworld', el => getComputedStyle(el).left);
        expect(boxTop).toBe('7px');
        expect(boxLeft).toBe('-5px');
    });
});
describe('Box 1', () => {
